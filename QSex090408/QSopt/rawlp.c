...
ILL_PTRWORLD_LEAKS_ROUTINE (colptr, colptr_check_leaks, this, int)
		 ...
		 static void ILLprt_EGlpNum (FILE * f,
																 EGlpNum_t * d)
{
										...
										int intmarker)
{
	...
	if (lp->ncols >= lp->colsize)
	{
		lp->colsize *= 1.3;
		lp->colsize +=1000;
		if(lp->colsize < lp->ncols+1) lp->colsize = lp->ncols+1;
	...
	if (lp->ncols >= lp->intsize)
	{
		lp->intsize *= 1.3;
		lp->intsize += 1000;
		if(lp->intsize < lp->ncols+1) lp->intsize = lp->ncols+1;
	...
	if (lp->ncols >= lp->is_sos_size)
	{
		lp->is_sos_size *= 1.3;
		lp->is_sos_size += 1000;
		if(lp->is_sos_size < lp->ncols+1) lp->is_sos_size = lp->ncols+1;
										...
										int tp)
{
	...
	if (lp->nsos >= lp->sos_setsize)
	{
		lp->sos_setsize *= 1.3;
		lp->sos_setsize += 1000;
		if(lp->sos_setsize < lp->nsos+1) lp->sos_setsize = lp->nsos+1;
										...
										EGlpNum_t rhs)
{
	...
	if (lp->nrows >= lp->sensesize)
	{
		lp->sensesize *= 1.3;
		lp->sensesize += 1000;
		if(lp->sensesize < lp->nrows+1) lp->sensesize = lp->nrows+1;
																	 ...
																	 EGlpNum_t bnd)
{
	...
	lp->ubind[i] = (char) 1;
	if (EGlpNumIsEqqual (lp->lower[i], zeroLpNum) &&
			EGlpNumIsEqqual (bnd, zeroLpNum))
	{
...
int ILLraw_fill_in_bounds (rawlpdata * lp)
{
	...
	for (i = 0; i < lp->ncols; i++)
	{
		if (!lp->lbind[i])
		{
			if (lp->ubind[i] && EGlpNumIsLess (lp->upper[i], zeroLpNum))
			{
				EGlpNumCopy (lp->lower[i], ILL_MINDOUBLE);
			}
		}
													...
													int i)
{
	...
	if (EGlpNumIsEqqual (lp->lower[i], zeroLpNum) &&
			EGlpNumIsLeq (zeroLpNum, lp->upper[i]))
	{
		return 1;
	}
	if (EGlpNumIsEqqual (lp->lower[i], ILL_MINDOUBLE) &&
			EGlpNumIsLess (lp->upper[i], zeroLpNum))
	{
		return 1;
	}
CLEANUP:
...
static int whichColsAreUsed (rawlpdata * raw,
														 ILLlpdata * lp,
														 int *colindex)
{
	int rval = 0;
...
static int whichRowsAreUsed (rawlpdata * raw,
														 ILLlpdata * lp,
														 int *rowindex)
{
	int i,
...
static int transferObjective (rawlpdata * raw,
															ILLlpdata * lp,
															int *colindex)
{
	int rval = 0,
...
static int transferSenseRhsRowNames (rawlpdata * raw,
																		 ILLlpdata * lp,
																		 int *rowindex)
{
	int i,
...
static int buildMatrix (rawlpdata * raw,
												ILLlpdata * lp,
												int *rowindex,
												int *colindex)
{
...
static int transferRanges (rawlpdata * raw,
													 ILLlpdata * lp,
													 int *rowindex)
{
	int i,
	...
	for (cp = raw->ranges; cp; cp = cp->next)
	{
		...
		case 'E':
			lp->sense[ri] = 'R';
			if (EGlpNumIsLeq (zeroLpNum, cp->coef))
			{
...
static int buildSosInfo (rawlpdata * raw,
												 ILLlpdata * lp,
												 int *colindex)
{
	int i,
...
static int convert_rawlpdata_to_lpdata (rawlpdata * raw,
																				ILLlpdata * lp)
/* 
...
int ILLrawlpdata_to_lpdata (rawlpdata * raw,
														ILLlpdata * lp)
{
	int rval = 0;
...
void ILLprint_rawlpdata (rawlpdata * lp)
{
	...
	if (lp)
	{
		...
		if (lp->ncols > 0)
		{
			...
			for (i = 0; i < lp->ncols; i++)
			{
				for (cp = lp->cols[i]; cp; cp = cp->next)
				{
					...
					printf ("%c ", (EGlpNumIsLess (cp->coef, zeroLpNum)) ? '-' : '+');
		...
		if (lp->rangesname)
		{
			...
			{
				printf ("(%s, %f) ", ILLraw_rowname (lp, cp->this),
								EGlpNumToLf (cp->coef));
			}
			printf ("\n");
			fflush (stdout);
