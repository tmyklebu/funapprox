...
#ifdef TRY_CODE
/* debug support */
static const char *get_str (const ILLsymboltab * h,
														int indx);
static void prt_xchain (const ILLsymboltab * h,
												int x);
static void prt_chain (const ILLsymboltab * h,
											 char *s);
#endif
												 ...
												 int init_size)
{
	...
	h->hashspace = ILLutil_nextprime (((unsigned)h->namespace));
#ifdef TRY_CODE
